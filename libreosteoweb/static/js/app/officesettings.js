  ...
  function($scope, $http, growl, OfficeSettingsServ, OfficeUsersServ, OfficePaimentMeansServ, i18nService, $uibModal, $q) {
    ...
    $scope.gridOptions = {
      ...
      columnDefs: [{
        ...
        cellTemplate: '<div class="ui-grid-cell-contents">{$ row.entity.is_staff|true_false$}</div>'
      }, {
        name: 'is_active',
        field: 'is_active',
        displayName: 'Actif',
        cellTemplate: '<div class="ui-grid-cell-contents">{$ row.entity.is_active|true_false$}</div>'
      }, {
        name: 'set_password',
        type: 'object',
        displayName: 'Mot de passe',
        allowCellFocus: false,
        cellTemplate: '<div class="ui-grid-cell-contents"><button ng-click="grid.appScope.setPassword(row.entity)">modifier</button></div>'
      }],
      data: $scope.users,
    };
    ...
    $scope.updateSettings = function(settings) {
      var update1 = OfficeSettingsServ.save({
        settingsId: $scope.officesettings.id
      }, settings);
      var updatePromises = [update1.$promise];
      angular.forEach($scope.paiment_means, function(paimentmean, key) {
        var updatePaimentMean = OfficePaimentMeansServ.save({
          paimentMeanId: paimentmean.id
        }, paimentmean);
